    ...
    public function generatePdf(Request $request)
    {
        ...
        try {
            ...
            $totalStockValue = $medications->sum(function ($med) {
                $qty = (int) ($med->quantity_on_hand ?? 0);
                $price = (float) ($med->current_sale_price ?? 0);
                return $qty * $price;
            });
            $lowStockCount = $medications->filter(function ($med) {
                $qty = (int) ($med->quantity_on_hand ?? 0);
                $reorder = (int) ($med->reorder_level ?? 0);
                return $qty <= $reorder;
            })->count();
            $outOfStockCount = $medications->filter(function ($med) {
                return ((int) ($med->quantity_on_hand ?? 0)) === 0;
            })->count();
            ...
            // For regular requests or AJAX, return the PDF for download
            return response($pdfContent)
                ->header('Content-Type', 'application/pdf')
                ->header('Content-Disposition', 'attachment; filename="' . $filename . '"');
        } catch (\Throwable $e) {
            Log::error('Stock report PDF generation failed', [
