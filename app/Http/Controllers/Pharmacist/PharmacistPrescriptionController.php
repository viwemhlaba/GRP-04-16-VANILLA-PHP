...
class PharmacistPrescriptionController extends Controller
{
    ...
    {
        // Correctly group conditions so ONLY approved prescriptions with remaining repeats are returned.
        $prescriptions = Prescription::with('user.customer', 'doctor', 'items.medication')
            ->where('status', 'approved')
            ->where(function ($query) {
                $query->whereHas('items', function ($sub) {
                    $sub->whereRaw('repeats_used < repeats');
                })
                ->orWhereRaw('repeats_used < repeats_total');
            })
            ->latest()
            ->get()
            ->map(function ($prescription) {
    ...
    public function dispenseStore(Request $request)
    {
        $validated = $request->validate([
            ...
            'items.*.item_id' => 'required_with:items|exists:prescription_items,id',
            'items.*.quantity' => 'required_with:items|integer|min:1',
            'items_json' => 'sometimes|string',
            'notes' => 'nullable|string|max:1000',
            'override_allergy' => 'sometimes|boolean',
        ]);

        // Support items passed as JSON string from frontend
        if ((!isset($validated['items']) || empty($validated['items'])) && $request->filled('items_json')) {
            $decoded = json_decode($request->input('items_json'), true);
            if (json_last_error() === JSON_ERROR_NONE && is_array($decoded)) {
                // Re-run a lightweight manual validation on structure
                $sanitized = [];
                foreach ($decoded as $row) {
                    if (!isset($row['item_id'], $row['quantity'])) continue;
                    $sanitized[] = [
                ...
                if (!empty($sanitized)) {
                    $validated['items'] = $sanitized;
                }
            }
        }

        if (empty($validated['items'] ?? [])) {
            return back()->withErrors(['items' => 'No items provided for dispensing.']);
        }

        $prescription = Prescription::with(['user.allergies.activeIngredient', 'items.medication.activeIngredients'])->findOrFail($validated['prescription_id']);
        ...
        // Allergy conflict check BEFORE dispensing
        $override = $request->boolean('override_allergy');
        $allergyAlerts = [];
        if ($prescription->user) {
            $service = new AllergyAlertService();
            // Collect the medications the pharmacist intends to dispense now
            $selectedItemIds = collect($validated['items'])->pluck('item_id');
            $medications = $prescription->items
                ->whereIn('id', $selectedItemIds)
                ->pluck('medication');

            $conflictResult = $service->checkMultipleMedicationConflicts($prescription->user, $medications->all());
            if ($conflictResult['has_any_conflicts']) {
                $allergyAlerts = $service->generateMultipleAlertMessages($conflictResult);
            }
        }

        if (!empty($allergyAlerts) && !$override) {
            // Return with alerts and flag so UI can prompt for confirmation
            return back()->with([
                'allergy_alerts' => $allergyAlerts,
        ...
        DB::transaction(function () use ($prescription, $validated) {
            ...
            foreach ($validated['items'] as $itemData) {
                ...
                // Validate that the quantity matches the original prescription quantity (cast to int to avoid type mismatch)
                if ((int) $quantity !== (int) $prescriptionItem->quantity) {
