...
class User extends Authenticatable
{
    ...
    /**
     * Global subject prefix for all outbound emails.
     */
    public const MAIL_SUBJECT_PREFIX = 'GRP-04-16';
    ...
    public function customer()
{
    ...
    /**
     * Send the password reset notification with group name in subject.
     */
    public function sendPasswordResetNotification($token): void
    {
        // Default Laravel notification class
        $this->notify(new class($token, static::MAIL_SUBJECT_PREFIX) extends \Illuminate\Notifications\Notification {
            public function __construct(private string $token, private string $prefix) {}
            ...
            public function toMail($notifiable): \Illuminate\Notifications\Messages\MailMessage
            {
                ...
                return (new \Illuminate\Notifications\Messages\MailMessage)
                    ->subject($this->prefix . ' Reset Your Password')
                    ->greeting('Password Reset Request')
                    ->line('You are receiving this email because we received a password reset request for your account.')
                    ->action('Reset Password', $url)
                    ->line('If you did not request a password reset, no further action is required.')
                    ->salutation('— ' . $this->prefix . ' System');
            }
        });
    }
}
