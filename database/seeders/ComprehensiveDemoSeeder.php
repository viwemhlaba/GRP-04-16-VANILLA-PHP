...
class ComprehensiveDemoSeeder extends Seeder
{
    ...
    {
    $dForms = DosageFormList::all();
    $tablet  = DosageForm::firstOrCreate(['name' => $dForms[0]]);
    $capsule = DosageForm::firstOrCreate(['name' => $dForms[1]]);
    $syrup   = DosageForm::firstOrCreate(['name' => $dForms[3]]);
    $cream   = DosageForm::firstOrCreate(['name' => $dForms[4]]); // Mapped Cream -> Lotion substitute if needed

        $supplier1 = MedicationSupplier::first() ?? MedicationSupplier::factory()->create();

        $canon = ActiveIngredientList::all();
        // Map demo medications onto first five canonical ingredients
        $medications = [
            [
                'name' => $canon[0] . ' 500mg',
                'schedule' => '2',
