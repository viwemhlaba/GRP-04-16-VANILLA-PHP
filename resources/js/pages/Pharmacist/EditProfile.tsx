...
const EditProfile = ({ pharmacist, pharmacies, specialization_options, language_options, license_status_options }: EditProfileProps) => {
    ...
    const validatePhoneNumber = (phoneNumber: string): string | null => {
        ...
        // SA phone number formats: +27XXXXXXXXX, 0XXXXXXXXX, or XXXXXXXXXX
        const saPhoneRegex = /^(\+27|0)?[6-8][0-9]{8}$/;
        if (!saPhoneRegex.test(phoneNumber.replace(/\s/g, ''))) {
            return 'Please enter a valid South African phone number (e.g., 0821234567 or +27821234567)';
        }
        return null;
    };

    const validateRegistrationNumber = (regNumber: string): string | null => {
        if (!regNumber) return null;
        // SA pharmacist registration format: A followed by 5-6 digits (e.g., A12345 or A123456)
        const saRegRegex = /^A\d{5,6}$/;
        if (!saRegRegex.test(regNumber)) {
            return 'Registration number must be in South African format (e.g., A12345 or A123456)';
        }
        return null;
    };
    ...
    const validateForm = (): boolean => {
        ...
        // Validate ID number
        const idError = validateIdNumber(data.id_number);
        ...
        if (regError) newErrors.registration_number = regError;
    ...
    const breadcrumbs = [
        { title: 'Profile', href: route('pharmacist.profile') },
        { title: !pharmacist.profile_completed ? 'Complete Profile' : 'Edit Profile', href: '#' },
    ];
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-6 p-6">
                <div className="space-y-1">
                    <h1 className="text-2xl font-semibold tracking-tight">{!pharmacist.profile_completed ? 'Complete Profile' : 'Edit Profile'}</h1>
                    <p className="text-muted-foreground text-sm">
                        {!pharmacist.profile_completed
                            ? 'Provide your professional and personal details to finish onboarding.'
                        ...
                        <CardContent className="space-y-4">
                            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Input id="surname" value={data.surname} onChange={(e) => handleInputChange('surname', e.target.value)} />
                                ...
                                <div className="space-y-2">
                                    ...
                                    {(validationErrors.phone_number || errors.phone_number) && (
                                        <p className="text-sm text-red-600">{validationErrors.phone_number || errors.phone_number}</p>
                                    )}
                                </div>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Input
                                        ...
                                        maxLength={13}
                                    />
                                    {(validationErrors.id_number || errors.id_number) && (
                                        <p className="text-sm text-red-600">{validationErrors.id_number || errors.id_number}</p>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                                <div className="space-y-2">
                                    ...
                                    <Input
                                        ...
                                        placeholder="A12345 or A123456"
                                    />
                                    {(validationErrors.registration_number || errors.registration_number) && (
                                        <p className="text-sm text-red-600">{validationErrors.registration_number || errors.registration_number}</p>
                                    )}
                                </div>
