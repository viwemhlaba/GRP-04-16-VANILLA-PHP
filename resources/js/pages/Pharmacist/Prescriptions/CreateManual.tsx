...
export default function CreateManualPrescription({ customers, doctors, medications, activeIngredients }: CreateManualPrescriptionProps) {
    ...
    const { data, setData, post, processing } = useForm<{
        customer_id: string;
        allergies_type: 'none' | 'known';
        allergies: number[];
        doctor_id: string;
        prescription_name: string;
        delivery_method: 'pickup' | 'dispense';
    ...
    const updateItem = (index: number, field: keyof PrescriptionLineItem, value: string | number) => {
        const newItems = [...data.items];
        newItems[index] = { ...newItems[index], [field]: value };

        // Update price when medication or quantity changes
        if (field === 'medication_id' || field === 'quantity') {
            const medicationId = (field === 'medication_id' ? value : newItems[index].medication_id) as string;
    ...
    const totalPrescriptionCost = data.items.reduce((acc: number, item) => acc + (item.price || 0), 0);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const validItems = data.items.filter((item) => item.medication_id && item.medication_id !== '');
    ...
    const breadcrumbs = [
        ...
        { title: 'Create Manual Prescription', href: route('pharmacist.prescriptions.createManual') },
    ];
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-6 p-6">
                ...
                <div className="space-y-1">
                    <h1 className="text-2xl font-semibold tracking-tight">Create Manual Prescription</h1>
                    <p className="text-muted-foreground text-sm">Manually capture a prescription with patient, doctor, medication and notes.</p>
                </div>
                                            ...
                                            {activeIngredients.map((ingredient) => (
                                                <div key={ingredient.id} className="flex items-center space-x-2">
                                                    <Checkbox
                                                        ...
                                                        checked={data.allergies.includes(ingredient.id)}
