...
// Form-friendly item with index signature to satisfy Inertia's FormDataConvertible typing
type FormPrescriptionItem = {
    medication_id: number | null;
    quantity: number;
    instructions: string;
    price?: number;
...
const LoadPrescription: React.FC<LoadPrescriptionProps> = ({
    ...
    // Define form data shape for generic typing
    interface FormDataShape {
        _method: string;
        customer_id: string;
        patient_id_number: string;
        doctor_id: string;
        items: FormPrescriptionItem[];
        status: Prescription['status'];
        reason_for_rejection: string;
        notes: string;
        repeats_total: number | null;
        [key: string]: string | number | boolean | null | FormPrescriptionItem[] | FormPrescriptionItem;
    }

    const { data, setData, post, processing, errors } = useForm<FormDataShape>({
        _method: 'put',
        customer_id: prescription.user_id?.toString() || '',
        patient_id_number: prescription.patient_id_number || '',
        doctor_id: prescription.doctor_id?.toString() || '',
        items: (prescription.is_manual && existingPrescriptionItems.length > 0
            ? existingPrescriptionItems
            : [{ medication_id: null, quantity: 1, instructions: '' }]) as FormPrescriptionItem[],
        status: prescription.status,
        reason_for_rejection: prescription.reason_for_rejection || '',
        notes: '',
        repeats_total: prescription.repeats_total ?? 0,
    });
    ...
    useEffect(() => {
        if (!data.items || data.items.length === 0) {
            setData('items', [{ medication_id: null, quantity: 1, instructions: '' }] as FormPrescriptionItem[]);
        }
    }, [data.items, setData]);

    // Helper functions for managing prescription items
    const updateItem = (index: number, field: keyof FormPrescriptionItem, value: string | number | null) => {
        const newItems = [...data.items];
        newItems[index] = { ...newItems[index], [field]: value } as FormPrescriptionItem;
        setData('items', newItems as FormPrescriptionItem[]);
    };

    const addItem = () => {
        setData('items', [...data.items, { medication_id: null, quantity: 1, instructions: '' }] as FormPrescriptionItem[]);
    };

    const removeItem = (index: number) => {
        const newItems = data.items.filter((_, i) => i !== index) as FormPrescriptionItem[];
    ...
    const totalPrescriptionCost = data.items.reduce((acc: number, item: PrescriptionItem) => acc + calculateTotalPrice(item), 0);
    ...
    const breadcrumbs = [
        ...
        { title: `Load #${prescription.id}`, href: route('pharmacist.prescriptions.load', prescription.id) },
    ];
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            {/* Wrapper adjusted: remove mx-auto centering to align content left like other pages (e.g., Manager Create forms) */}
            <div className="max-w-4xl space-y-6 px-4 py-6 sm:px-6 lg:px-8">
                <div className="space-y-1">
                    <h1 className="text-2xl font-semibold tracking-tight">Prescription #{prescription.id}</h1>
                    <p className="text-muted-foreground text-sm">Review and manage this prescription's details and status.</p>
                </div>

                <Card className="border-border/60 rounded-none border shadow-sm">
                    <CardHeader>
                            ...
                            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <Select
                                            ...
                                            onValueChange={(value) => {
                                                ...
                                                if (selectedCustomer?.id_number) setData('patient_id_number', selectedCustomer.id_number);
                                            }}
                                        >
                                            <SelectTrigger>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Input
                                        id="repeats_total"
                                        type="number"
                                        value={data.repeats_total === null ? '' : data.repeats_total}
