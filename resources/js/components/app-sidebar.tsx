...
function NavItemComponent({ item }: NavItemComponentProps) {
    ...
    return (
        <Link
            href={item.href!}
            className="group hover:bg-accent hover:text-accent-foreground text-muted-foreground flex w-full items-center rounded-md px-2 py-1.5 text-[13px] font-semibold tracking-wide"
        >
            {item.icon && <item.icon className="mr-2 h-4 w-4 opacity-80 transition group-hover:opacity-100" />}
...
export function AppSidebar() {
    ...
    // Allow other arbitrary inertia props without strict typing
    ...
    const mainNavItems = [...sharedNav, ...(role ? (roleNavMap[role] ?? []) : [])];
