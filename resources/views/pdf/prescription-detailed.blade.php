...
<body>
    ...
    <div class="brand">iBHAYI E-PRESCRIPTION</div>
        <h1>Prescription</h1>
        <div class="meta-band">
            <span class="badge">#{{ $prescription->id }}</span>
            <span class="badge outline">{{ $prescription->is_manual ? 'Manual Entry' : 'Uploaded' }}</span>
            <span class="badge outline">{{ strtoupper($prescription->delivery_method) }}</span>
            <span class="badge {{ $prescription->repeats_used >= $prescription->repeats_total ? 'warn' : 'outline' }}">REPEATS {{ $prescription->repeats_used }} / {{ $prescription->repeats_total }}</span>
            <span class="badge status-{{ $prescription->status }}">{{ strtoupper($prescription->status) }}</span>
        </div>
        @if($current_pharmacist && $current_pharmacist->pharmacy)
            <p class="small muted" style="margin-top:6px;">{{ $current_pharmacist->pharmacy->name }} — Reg: {{ $current_pharmacist->pharmacy->registration_number ?? 'N/A' }}</p>
        @endif
        <p class="small muted" style="margin-top:2px;">Created: {{ $prescription->created_at->format('M d, Y H:i') }} @if($prescription->next_repeat_date) | Next Repeat: {{ \Carbon\Carbon::parse($prescription->next_repeat_date)->format('M d, Y') }} @endif</p>
    </div>
    ...
    <div class="section">
        ...
        <table class="kv-table">
            <tr>
                ...
                <td class="wrap">{{ $prescription->name }}</td>
            </tr>
            <tr>
                <td class="kv-label">Status</td>
                <td>{{ strtoupper($prescription->status) }}</td>
            </tr>
            <tr>
                <td class="kv-label">Repeats Used</td>
                <td>{{ $prescription->repeats_used }} / {{ $prescription->repeats_total }}</td>
            </tr>
        ...
        <table class="grid-3">
            <tr>
                <td>
                    ...
                    <div><span class="label">Name:</span> {{ $patient->name }} {{ $patient->surname }}</div>
                    <div><span class="label">ID:</span> {{ $customer->id_number ?? $prescription->patient_id_number ?? 'N/A' }}</div>
                    <div><span class="label">Email:</span> {{ $patient->email }}</div>
                    <div><span class="label">Phone:</span> {{ $customer->cellphone_number ?? 'N/A' }}</div>
                    @if($customer && ($customer->address_line_1 || $customer->address_line_2 || $customer->city))
                        <div><span class="label">Address:</span><br>
                            <span class="value small">
                                ...
                                @if($customer->city){{ $customer->city }}@endif @if($customer->postal_code), {{ $customer->postal_code }}@endif
                            </span>
                        </div>
                    @endif
                </td>
                <td>
                    ...
                    <div><span class="label">Name:</span> Dr. {{ $doctor->name }} {{ $doctor->surname }}</div>
                    <div><span class="label">Practice #:</span> {{ $doctor->practice_number }}</div>
                    @if($doctor->email)<div><span class="label">Email:</span> {{ $doctor->email }}</div>@endif
                    @if($doctor->phone)<div><span class="label">Phone:</span> {{ $doctor->phone }}</div>@endif
                    @if($doctor->specialization)<div><span class="label">Specialization:</span> {{ $doctor->specialization }}</div>@endif
                </td>
                <td>
                    <h3 style="font-size:11px; margin:0 0 6px; color:var(--accent);">Pharmacist</h3>
                    @if($current_pharmacist)
                        <div><span class="label">Name:</span> {{ $current_pharmacist->user->name ?? 'N/A' }} {{ $current_pharmacist->user->surname ?? '' }}</div>
                        @if($current_pharmacist->license_number)<div><span class="label">License #:</span> {{ $current_pharmacist->license_number }}</div>@endif
                        @if($current_pharmacist->pharmacy)<div><span class="label">Pharmacy:</span> {{ $current_pharmacist->pharmacy->name }}</div>@endif
                        @if($current_pharmacist->pharmacy && $current_pharmacist->pharmacy->phone)<div><span class="label">Phone:</span> {{ $current_pharmacist->pharmacy->phone }}</div>@endif
                        @if($current_pharmacist->pharmacy && $current_pharmacist->pharmacy->address)<div><span class="label">Address:</span><br><span class="value small">{{ $current_pharmacist->pharmacy->address }}</span></div>@endif
                    @else
        ...
        <table>
            ...
            @foreach($items as $item)
                <tr>
                    ...
                    <td class="small">{{ $item->medication->activeIngredients->pluck('name')->join(', ') ?: 'N/A' }}</td>
            ...
            @endforeach
                <tr class="totals-row">
                    ...
                    <td class="text-right">R{{ number_format($totals['prescription_total_value'], 2) }}</td>
                </tr>
            </tbody>
        </table>
    </div>
    ...
    <div class="section">
        ...
        <table>
            ...
            @foreach($dispensed_history as $item)
                <tr>
                    <td class="small">{{ $item->dispensed_at->format('M d, Y') }}<br><span class="muted small">{{ $item->dispensed_at->format('H:i') }}</span></td>
                    <td>{{ $item->medication->name }}</td>
                    <td class="text-center">{{ $item->quantity_dispensed }}</td>
                    <td class="small">{{ $item->pharmacist->name ?? 'Unknown' }}</td>
                    <td class="small">{{ $item->pharmacist->pharmacistProfile->pharmacy->name ?? 'Unknown Pharmacy' }}</td>
                    <td class="text-right">R{{ number_format($item->cost, 2) }}</td>
                </tr>
            @endforeach
                <tr class="totals-row">
                    <td colspan="5" class="text-right">Total Dispensed Value</td>
                    <td class="text-right">R{{ number_format($totals['total_cost'], 2) }}</td>
                </tr>
            </tbody>
        </table>
        <div class="summary-bar">
            <span><strong>Summary</strong></span>
            <span>Total Items Dispensed: {{ $totals['total_items_dispensed'] }}</span>
        </div>
    </div>
    @endif
    ...
    <div class="footer">
        Document generated {{ $generated_at->format('F d, Y H:i:s') }} &mdash; This is a computer generated record. If repeats are exhausted, a new prescription is required.
    </div>
</body>
</html>
